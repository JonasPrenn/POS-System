...
| **3 — Geld** | Deckelabrechnung mit PDF, E-Mail und QR; Zahlungseingang und Kontoauszug-Import; Erinnerungen. Schichten, Entnahmen, Kassenbuch (mit den App-Bildschirmen). *Stand 22. September 2026: gebaut. Abrechnung — Lauf, PDF mit EPC-QR, Versand, CAMT/CSV-Import, Zuordnung, Erinnerung, Storno; eine Aufladung an der Theke nach dem Stichtag steht an der Abrechnung und auf der Übersicht, ein Klick schließt sie damit ohne zweite Buchung. Kasse — Schicht öffnen, Entnahme und Einlage, Schließen mit Zählung und Differenzvermerk am Tablet (`CashSection`, Schema 13); im Web Tagesbericht je Gerät, laufende Schichten, Kassenbuch mit fortgeführtem Bestand und CSV, Bankbuch. Offen: das Kassenbuch als PDF, der Storno einer Bewegung per Gegenbuchung (heute: eine zweite Bewegung mit Grund).* | Der eigentliche Nutzen. Braucht Profile (2) und den unveränderlichen Buchungsstrom (0). |
