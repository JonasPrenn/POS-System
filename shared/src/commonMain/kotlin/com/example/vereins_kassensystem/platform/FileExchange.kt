 ...
 * regelt.
 *
 * Die App tauscht über Dateien nur Text aus: Produkt- und Mitgliederlisten als CSV. Der
 * Vertrag reicht deshalb Zeichenketten und keine Ströme durch — die Ströme waren der
 * Grund, warum `ProductViewModel` und `MemberViewModel` `java.io.InputStream` in der
 * Signatur trugen und damit an die JVM gebunden waren. Eine CSV mit Vereinsprodukten
 * passt in den Speicher; für etwas anderes ist dieser Weg ohnehin nicht gedacht.
 *
 * Die Sicherungsdatei ist der Sonderfall — sie ist ein Zip und geht deshalb über
 * [BackupExchange] statt hier durch.
 */
interface FilePicker {
    /**
     * Öffnet die Dateiauswahl des Systems.
     *
     * [suggestedName] ist nur ein Vorschlag; beide Plattformen lassen den Nutzer
     * umbenennen, und unter iOS kann die Datei in iCloud landen statt auf dem Gerät.
     */
    fun open()
}
 ...
 *
 * Der Inhalt wird beim Öffnen übergeben und nicht erst beim Bestätigen erzeugt: Zwischen
 * dem Tippen auf "Export" und dem Bestätigen im Systemdialog können Sekunden liegen, und
 ...
 *
 * Der Verweis ist plattformabhängig — unter Android eine Content-URI, unter iOS ein Pfad
 * im App-Container. Er wird deshalb nie zum Server geschickt: Die Spezifikation sieht
 * dafür einen Upload und einen serverseitigen Schlüssel vor, weil eine Android-URI auf
 * einem iPad nichts bedeutet.
 */
@Composable
...
/** Lässt den Ablageort für Sicherungen wählen und merkt ihn dauerhaft. */
@Composable
expect fun rememberBackupDestinationPicker(onChosen: (String?) -> Unit): FilePicker

/** Lässt eine Sicherungsdatei zum Einspielen auswählen. */
@Composable
